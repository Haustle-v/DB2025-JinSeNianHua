...
std::shared_ptr<Query> Analyze::do_analyze(
    ...
    if (auto x = std::dynamic_pointer_cast<ast::SelectStmt>(parse)) {
        ...
        if (query->join_type_ == JoinType::ANTI_JOIN) {
            get_all_cols_of_left_tab(x->jointree[0]->left,
                                     all_cols_of_left_tab);
            ...
            if (query->cols.empty()) {  // select * 表示 select all
                ...
                for (auto& sel_col : query->cols) {
                    sel_col = check_column4semi_join(all_cols_of_left_tab,
                                                     sel_col);  // 列元数据校验
                }
            }
        } else {
        ...
        check_clause(
            query->tables,
            query->join_conds);  // 检查列名是否存在，以及可能需要推断表名
...
TabCol Analyze::check_column(const std::vector<ColMeta>& all_cols,
    ...
    if (target.tab_name.empty()) {
        ...
        // sqb: down! 5.24
        if (!sm_manager_->db_.get_table(target.tab_name)
                 .is_col(target.col_name)) {
            throw ColumnNotFoundError(target.col_name);
        }
    }
    return target;
...
TabCol Analyze::check_column4semi_join(
    const std::vector<ColMeta>& all_cols,  // 左表的所有列
    TabCol
        target) {  // select的某一列（这只是一列，对select的列的遍历在函数外）
...
void Analyze::get_clause(
    ...
    for (auto& expr : sv_conds) {
        ...
        cond.lhs_col = {
            .tab_name =
                expr->lhs
                    ->tab_name,  // 这个是C++20的写法，但我现在C++17为啥没报错？
