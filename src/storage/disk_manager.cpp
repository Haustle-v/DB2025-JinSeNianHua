...
void DiskManager::write_page(int fd, page_id_t page_no, const char *offset, int num_bytes) {
  ...
  // 使用 unique_lock 配合 shared_mutex 获取独占锁（写锁）
  std::unique_lock<std::shared_mutex> file_lock(*iter->second);
...
void DiskManager::read_page(int fd, page_id_t page_no, char *offset, int num_bytes) {
  ...
  // 使用 shared_lock 获取共享锁（读锁），允许多个读线程并发
  std::shared_lock<std::shared_mutex> file_lock(*iter->second);
...
void DiskManager::create_file(const std::string &path) {
  ...
  if (fd == -1) {
    // 感觉unix error应该更合适一点 但主打一个方便找错
    throw InternalError("DiskManager create file failed");
  }
  close(fd);
}
...
int DiskManager::open_file(const std::string &path) {
  ...
  //   为文件注册读写锁
  fd_locks_[fd] = std::make_shared<std::shared_mutex>();
...
int DiskManager::read_log(char *log_data, int size, int offset) {
  ...
  // 注意：此函数和 write_log 共享 log_fd_，但没有应用文件锁保护
  // 如果多个线程可能同时调用 read_log 和 write_log，这里存在潜在的竞争条件
  if (log_fd_ == -1) {
