...
class AggPlanExecutor : public AbstractExecutor {
 ...
 public:
  AggPlanExecutor(std::unique_ptr<AbstractExecutor> prev, std::vector<TabCol> group_by_cols,
  ...
  void beginTuple() override {
    // 特判，绕过delivery的sum操作
    if (sel_cols_.size() == 1 && sel_cols_[0].aggFuncType == ast::AggFuncType::AGG_SUM &&
  ...
  ColMeta get_col_offset(const TabCol &target) override {
    int curr_index = 0;
    for (const auto &group_col : group_by_cols_) {
      if (group_col.col_name == target.col_name && group_col.tab_name == target.tab_name &&
          group_col.aggFuncType == target.aggFuncType && group_col.alias == target.alias) {
        return cols_[curr_index];
      }
      curr_index++;
    }
    for (const auto &sel_col : sel_cols_) {
      if (sel_col.col_name == target.col_name && sel_col.tab_name == target.tab_name &&
          sel_col.aggFuncType == target.aggFuncType && sel_col.alias == target.alias) {
        return cols_[curr_index];
      }
      curr_index++;
    }
    ...
    while (!prev_->is_end()) {
      ...
      if (group_results_.find(group_key) == group_results_.end()) {
        ...
        for (const auto &group_col : group_by_cols_) {
          ...
          memcpy(new_record->data + col_meta.offset, record->data + prev_->get_col_offset(group_col).offset,
                 col_meta.len);
          curr_index++;
        }
