...
class SortExecutor : public AbstractExecutor {
 ...
 public:
  SortExecutor(std::unique_ptr<AbstractExecutor> prev, std::vector<TabCol> order_by_cols, std::vector<bool> is_asc,
               int32_t limit)
      : prev_(std::move(prev)),
  ...
  void beginTuple() override {
    ...
    auto comparator = [this](const std::unique_ptr<RmRecord> &a, const std::unique_ptr<RmRecord> &b) {
      for (size_t i = 0; i < order_by_cols_.size(); i++) {
        ...
        int cmp = ix_compare(a->data + col_meta.offset, b->data + col_meta.offset, col_meta.type, col_meta.len);
        if (cmp != 0) {
          // is_asc ? (a < b) : (a > b)
