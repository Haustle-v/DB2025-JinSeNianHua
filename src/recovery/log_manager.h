...
class CkptLogRecord : public LogRecord {
 public:
  ...
  CkptLogRecord(txn_id_t txn_id) : CkptLogRecord() { log_tid_ = txn_id; }
...
class InsertLogRecord : public LogRecord {
 public:
  ...
  InsertLogRecord(txn_id_t txn_id, RmRecord &insert_value, const Rid &rid, std::string table_name) : InsertLogRecord() {
    log_tid_ = txn_id;
...
class DeleteLogRecord : public LogRecord {
 public:
  ...
  DeleteLogRecord(txn_id_t txn_id, RmRecord &delete_value, const Rid &rid, std::string table_name) : DeleteLogRecord() {
    log_tid_ = txn_id;
...
class UpdateLogRecord : public LogRecord {
 public:
  ...
  UpdateLogRecord(txn_id_t txn_id, RmRecord &old_value, RmRecord &new_value, const Rid &rid, std::string table_name)
      : UpdateLogRecord() {
    log_tid_ = txn_id;
...
class LogManager {
 public:
  ...
  //   sqb 6.11 恢复后重新设置lsn
  void set_global_lsn(lsn_t log_max_lsn) { global_lsn_ = log_max_lsn; }

 private:
  //   sqb 辅助刷盘 6.5
  void flush_log2disk_helper();
  ...
  uint64_t checkpoint_pos_{0};  // checkpoint_lsn_对应日志记录的下一记录起始 sqb 6.11
};
