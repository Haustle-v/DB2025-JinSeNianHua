 ...
 public:
  ...
  std::unique_ptr<AbstractExecutor> convert_plan_executor(std::shared_ptr<Plan> plan, Context *context) {
    if (auto x = std::dynamic_pointer_cast<ProjectionPlan>(plan)) {
      ...
      return std::make_unique<SortExecutor>(std::move(subplan), std::move(x->sel_cols_), std::move(x->is_asc_),
                                            x->limit_);
    } else if (auto x = std::dynamic_pointer_cast<AggPlan>(plan)) {
      return std::make_unique<AggPlanExecutor>(convert_plan_executor(x->subplan_, context), x->group_by_cols,
                                               x->sel_cols_);
    } else if (auto x = std::dynamic_pointer_cast<HavingPlan>(plan)) {
      return std::make_unique<HavingPlanExecutor>(convert_plan_executor(x->subplan_, context), x->having_conds_);
    }
    return nullptr;
  }
};