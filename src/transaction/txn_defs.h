...
class WriteRecord {
 public:
  ...
  WriteRecord(WType wtype, const std::string &tab_name, const Rid &rid, const TupleMeta &meta)
      : wtype_(wtype), tab_name_(tab_name), rid_(rid), meta_(meta) {}

  // constructor for delete & update operation
  WriteRecord(WType wtype, const std::string &tab_name, const Rid &rid, const RmRecord &record, const TupleMeta &meta)
      : wtype_(wtype), tab_name_(tab_name), rid_(rid), record_(record), meta_(meta) {}
  ...
  inline TupleMeta &GetTupleMeta() { return meta_; }
 ...
 private:
  ...
  Rid rid_;
  RmRecord record_;
  TupleMeta meta_;  // sqb 增加tuplemeta MVCC情景下回滚用
};
