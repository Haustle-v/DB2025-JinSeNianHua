...
class Planner {
 ...
 private:
  ...
  std::shared_ptr<Plan> generate_agg_plan(std::shared_ptr<Query> query, std::shared_ptr<Plan> plan);
  std::shared_ptr<Plan> generate_having_plan(std::shared_ptr<Query> query, std::shared_ptr<Plan> plan);
  ...
  //   sqb 根据表大小做排序
  void sort_table_on_num(std::vector<std::string> &query_tables);
