        ...
        while (it != conds.end()) {
            ...
            if (std::find(joined_tables.begin(), joined_tables.end(),
                ...
                left_need_to_join_executors =
                    pop_scan(scantbl, it->lhs_col.tab_name, joined_tables,
                             table_scan_executors);
            }
            if (std::find(joined_tables.begin(), joined_tables.end(),
                          it->rhs_col.tab_name) == joined_tables.end()) {
                right_need_to_join_executors =
                    pop_scan(scantbl, it->rhs_col.tab_name, joined_tables,
                             table_scan_executors);
                isneedreverse = true;
            }
...
std::shared_ptr<Plan> Planner::generate_having_plan(
    std::shared_ptr<Query> query,
    ...
    if (!x->having_conds.empty() && x->has_agg) {
        plan = std::make_shared<HavingPlan>(T_Having, std::move(plan),
                                            std::move(query->having_conds));
    }
    return plan;
}
...
std::shared_ptr<Plan> Planner::generate_agg_plan(std::shared_ptr<Query> query,
    ...
    // 生成聚合计划
    plan = std::make_shared<AggPlan>(T_Agg, std::move(plan),
                                     std::move(query->group_by_cols),
                                     std::move(query->cols));
    return plan;
}
...
std::shared_ptr<Plan> Planner::generate_sort_plan(std::shared_ptr<Query> query,
    ...
    plan = std::make_shared<SortPlan>(
        T_Sort, std::move(plan), std::move(query->order_bys.cols),
...
std::shared_ptr<Plan> Planner::do_planner(std::shared_ptr<Query> query,
    ...
    if (auto x = std::dynamic_pointer_cast<ast::CreateTable>(query->parse)) {
        ...
        for (auto& field : x->fields) {
            if (auto sv_col_def =
                    std::dynamic_pointer_cast<ast::ColDef>(field)) {
                ColDef col_def = {
        ...
        plannerRoot = std::make_shared<DDLPlan>(
            T_CreateTable, x->tab_name, std::vector<std::string>(), col_defs);
                   ...
                   std::dynamic_pointer_cast<ast::DropTable>(query->parse)) {
        // drop table;
        ...
        plannerRoot = std::make_shared<DDLPlan>(
            T_CreateIndex, x->tab_name, x->col_names, std::vector<ColDef>());
                   ...
                   std::dynamic_pointer_cast<ast::DropIndex>(query->parse)) {
        // drop index
        plannerRoot = std::make_shared<DDLPlan>(
        ...
        if (index_exist == false) {  // 该表没有索引
            index_col_names.clear();
            table_scan_executors =
                std::make_shared<ScanPlan>(T_SeqScan, sm_manager_, x->tab_name,
                                           query->conds, index_col_names);
        } else {  // 存在索引
            table_scan_executors = std::make_shared<ScanPlan>(
                T_IndexScan, sm_manager_, x->tab_name, query->conds,
                index_col_names);
        }
        ...
        plannerRoot = std::make_shared<DMLPlan>(
            T_Update, table_scan_executors, x->tab_name, std::vector<Value>(),
